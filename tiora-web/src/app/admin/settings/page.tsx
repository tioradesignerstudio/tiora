...
import { Save, Loader2, Upload, Sparkles, AlertCircle, Check, Image as ImageIcon, Plus, Trash2, Megaphone, ChevronDown } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [bannerSelectedLink, setBannerSelectedLink] = useState("");
        ...
        if (val) {
          try {
            if (val.trim().startsWith("[")) {
              setBanners(JSON.parse(val));
            } else {
              setBanners([{ url: val, link: "" }]);
            }
          } catch {
            setBanners([{ url: val, link: "" }]);
          }
  ...
  const handleAddBanner = (e: React.FormEvent) => {
    ...
    const finalLink = bannerSelectedLink === "__custom__" ? customLinkInput.trim() : bannerSelectedLink;
    setBanners((prev) => [...prev, { url: bannerInputUrl.trim(), link: finalLink }]);
    setBannerInputUrl("");
    setBannerSelectedLink("");
    setCustomLinkInput("");
    setSuccess("Banner added locally. Click Save Banners Setting below to save!");
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-10">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-brand/5">
            <h2 className="text-xl font-playfair font-bold text-brand mb-6 border-b border-brand/5 pb-4">Homepage Hero Banner Carousel</h2>
            ...
            <form onSubmit={handleSaveBanner} className="space-y-6">
              <div className="flex gap-4 items-end">
                ...
                <button
                  ...
                  className="bg-[#0E2C2C] text-[#C5A059] px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#0A2222] transition-colors h-[54px] flex items-center justify-center gap-1 shadow-md cursor-pointer"
                >
                  <Plus size={14} /> Add
                </button>
              </div>

              <div className="bg-brand/5 border border-transparent rounded-2xl p-5 space-y-4">
                <div>
                  <label className="block text-[10px] font-black text-brand/40 uppercase tracking-[0.2em] mb-3 ml-1">Banner Click Navigation Link (Optional)</label>
                  <div className="relative">
                    <select
                      value={bannerSelectedLink}
                      onChange={(e) => setBannerSelectedLink(e.target.value)}
                      className="w-full bg-white text-xs font-bold text-[#0E2C2C] border border-black/10 rounded-2xl pl-5 pr-10 py-4 focus:outline-none focus:ring-1 focus:ring-[#C5A059] cursor-pointer appearance-none shadow-sm"
                    >
                ...
                {bannerSelectedLink === "__custom__" && (
                  <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                    <label className="block text-[10px] font-black text-brand/40 uppercase tracking-[0.2em] mb-3 ml-1">Custom Link URL</label>
                    <input
                      type="text"
                      value={customLinkInput}
                      onChange={(e) => setCustomLinkInput(e.target.value)}
                      placeholder="e.g. /my-story or https://example.com"
                      className="w-full bg-white border border-black/10 focus:border-[#C5A059]/50 rounded-2xl px-5 py-4 text-xs font-semibold text-brand outline-none transition-all placeholder:text-brand/20 shadow-sm"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="mt-8 border-t border-brand/5 pt-6">
                <h3 className="text-xs font-black uppercase tracking-wider text-brand/50 mb-4">
                  Configured Banner Images ({banners.length})
                </h3>
                {banners.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {banners.map((b, idx) => (
                      <div key={idx} className="relative border border-brand/10 bg-brand/5 rounded-2xl p-3 flex flex-col group overflow-hidden">
                        <div className="aspect-[21/9] w-full rounded-xl overflow-hidden bg-white mb-3 relative">
                          <img src={b.url} alt={`Slide ${idx + 1}`} className="w-full h-full object-cover" />
                          <button
                            type="button"
                            onClick={() => setBanners((prev) => prev.filter((_, i) => i !== idx))}
                            className="absolute top-2 right-2 bg-white hover:bg-red-50 text-red-500 p-2 rounded-xl border border-red-100 shadow-md cursor-pointer transition-all hover:scale-105 active:scale-95"
                            title="Remove Banner"
                          >
                            <Trash2 size={14} />
                          </button>
                        ...
                        <div className="px-1 text-xs">
                          <p className="font-bold text-brand truncate" title={b.url}>
                            URL: <span className="font-medium text-brand/60">{b.url}</span>
                          </p>
                          <p className="font-bold text-brand truncate mt-1">
                            Link: <span className="font-medium text-[#B18E35]">{b.link || "No Link"}</span>
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  <p className="text-xs italic text-brand/40 text-center py-6">No banner images configured yet.</p>
                )}
              </div>

              <button
                type="submit"
                disabled={isSubmitting || isUploading || banners.length === 0}
                className="w-full flex items-center justify-center space-x-2 bg-[#1B3022] text-[#C5A059] py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#2c4d37] transition-all shadow-lg disabled:opacity-50 mt-6"
              >
                {isSubmitting ? (
                ...
                <span>Save Banners Setting</span>
              </button>
            </form>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-brand/5 flex flex-col h-fit">
          ...
          <div className="rounded-2xl border border-brand/10 overflow-hidden relative flex flex-col p-0 bg-brand/5">
            ...
            {banners.length > 0 ? (
              <div className="relative w-full overflow-hidden bg-white aspect-[21/9]">
                <img
                  src={banners[0].url}
                  alt="Banner Preview (First Slide)"
                  className="w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-black/5 pointer-events-none"></div>
                {banners.length > 1 && (
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/50 backdrop-blur-sm text-white text-[9px] font-bold uppercase px-3 py-1 rounded-full tracking-widest">
              ...
              <div className="h-32 flex flex-col items-center justify-center text-brand/30 border-dashed bg-white space-y-1">
                <ImageIcon size={20} />
                <p className="text-[9px] font-bold uppercase tracking-widest">No Promo Banners Configured</p>
              </div>
            )}
          </div>
        </div>
