      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 bg-white rounded-[2.5rem] p-8 shadow-sm border border-brand/5 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-brand/5 border border-transparent focus:border-[#C5A059]/50 rounded-none px-5 py-3.5 text-sm font-semibold text-brand outline-none transition-all placeholder:text-brand/20"
                />
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-[#B18E35]/30 hover:border-[#B18E35] bg-[#faf8f0] p-6 text-center cursor-pointer transition-colors group rounded-none">
                  {isUploading ? (
                    <div className="flex flex-col items-center gap-2 py-2">
                      <Loader2 className="w-6 h-6 animate-spin text-[#B18E35]" />
                      <span className="text-xs font-bold text-brand">Uploading image...</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center py-2">
                      <Upload className="w-8 h-8 text-[#B18E35] mb-2 transition-transform group-hover:-translate-y-0.5" />
                      <span className="text-sm font-bold text-neutral-700">Click or Drag Files here</span>
                      <span className="text-[10px] text-neutral-400 mt-1">Supports Images (PNG, JPG, WEBP)</span>
                    </div>
                  )}
                  <input
                    type="file"
            ...
            <div>
              <label className="block text-[10px] font-black text-brand/40 uppercase tracking-[0.2em] mb-2 ml-1 whitespace-nowrap">
                Promo/Discount Matter (Optional)
              </label>
              <input
                type="text"
                value={promoText}
                onChange={(e) => setPromoText(e.target.value)}
                placeholder="e.g. 50-80% OFF (Optional)"
                className="w-full bg-brand/5 border border-transparent focus:border-[#C5A059]/50 rounded-none px-5 py-3.5 text-sm font-semibold text-brand outline-none transition-all placeholder:text-brand/20"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
              <div>
                <label className="block text-[10px] font-black text-brand/40 uppercase tracking-[0.2em] ml-1">
                  Action Button Text
                </label>
              </div>
              <div>
            ...
            <div className="border border-black/10 rounded-none p-4 bg-white">
              <span className="block text-[8px] font-semibold text-brand/40 uppercase tracking-[0.2em] mb-3 ml-1">
                Live Card Mockup Preview
              </span>
              
              <div className="flex flex-col items-center mx-auto w-48 bg-white rounded-none">
                <div className="w-full aspect-[2/3] overflow-hidden relative shadow-none border border-black/10">
                  {imageUrl ? (
                    <img
                      src={imageUrl}
                      alt="Mockup Preview"
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-brand/20">
                      <ImageIcon size={32} />
                      <span className="text-[9px] uppercase tracking-widest font-bold mt-2">No Image</span>
                    </div>
                  )}
                </div>
                <span className="mt-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#111111] text-center">
                  {name || "Category Name"}
                </span>
              </div>
            </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white rounded-none p-8 border border-black/10 shadow-none">
            <h2 className="text-xl font-playfair font-bold text-brand mb-6 border-b border-black/5 pb-4">
              Active Category Cards ({categories.length})
            </h2>
 
            {categories.length === 0 ? (
              <div className="text-center py-16 border border-dashed border-black/10 rounded-none bg-neutral-50 flex flex-col items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {categories.map((item, index) => (
                  <div 
                    key={item.id} 
                    className="relative border border-black/10 bg-white rounded-none p-5 flex flex-col group transition-all duration-300"
                  >
                    {/* Visual Card Display */}
                    <div className="flex gap-4">
                      <div className="w-24 aspect-[2/3] bg-white rounded-none overflow-hidden border border-black/10 relative flex-shrink-0">
                        <img 
                          src={item.imageUrl} 
                          alt={item.name} 
                          className="w-full h-full object-cover" 
                      ...
                      <div className="flex-1 flex flex-col justify-between py-1 min-w-0">
                        <div>
                          ...
                          <p className="text-[#333333]/60 text-xs font-semibold uppercase tracking-wider mt-1">{item.promoText}</p>
                        ...
                        <div className="flex flex-col gap-2 mt-4">
                          ...
                          <div className="flex items-center space-x-2">
                            <button
                              ...
                              className="p-2.5 bg-white text-brand hover:text-brand-accent rounded-none border border-black/10 hover:scale-105 transition-all cursor-pointer"
                              title="Edit Card"
                            >
                              <Edit3 size={14} />
                            </button>
                            <button
                              ...
                              className="p-2.5 bg-white text-red-500 hover:bg-red-50 rounded-none border border-black/10 hover:scale-105 transition-all cursor-pointer"
                              title="Delete Card"
                            >
                              <Trash2 size={14} />
                            </button>
                          ...
                          {/* Row 2: Reorder Up & Down */}
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() => handleReorder(index, "up")}
                              disabled={index === 0}
                              className="p-2.5 bg-white text-brand hover:text-brand-accent rounded-none border border-black/10 hover:scale-105 transition-all disabled:hover:scale-100 disabled:opacity-30 cursor-pointer"
                              title="Move Up"
                            >
                              <ArrowUp size={14} />
                            </button>
