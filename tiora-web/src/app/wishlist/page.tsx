...
import { Heart, Trash2, X, Check, ArrowRight, Loader2 } from "lucide-react";
...
export default function WishlistPage() {
  ...
  if (loadingAuth || !isLoaded) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-[#faf8f0]">
        <div className="flex flex-col items-center space-y-4">
          <Loader2 className="w-12 h-12 animate-spin text-[#333333]" />
          <p className="text-xs font-black uppercase tracking-widest text-[#333333]/60">Loading Wishlist...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f0] text-[#1B3022] font-sans">
      <main className="max-w-7xl mx-auto px-6 md:px-12 py-16">
        ...
        {wishlistItems.length === 0 ? (
          <div className="text-center py-20 bg-[#faf8f0] rounded-[2.5rem] border border-brand/5 shadow-sm max-w-xl mx-auto px-8">
            <Heart className="mx-auto mb-6 opacity-20 text-[#333333] animate-pulse" size={56} />
            ...
            <Link
              href="/"
              className="inline-flex items-center space-x-2 text-xs font-bold uppercase tracking-widest bg-[#333333] text-white px-8 py-4 rounded-xl hover:bg-[#32451B] transition shadow-lg"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {wishlistItems.map((item) => {
              ...
              return (
                <div
                  key={item.productId}
                  className="group flex flex-col w-full bg-[#faf8f0] rounded-3xl border border-brand/5 shadow-sm hover:shadow-md hover:scale-[1.02] transition-all duration-300 overflow-hidden relative"
                >
                  {/* Delete Button */}
                  <button
                    onClick={() => removeItem(item.productId)}
                    className="absolute top-3 right-3 z-10 p-2 bg-[#faf8f0] hover:bg-red-50 text-[#1B3022]/60 hover:text-red-500 rounded-full border border-brand/5 transition-all shadow-sm cursor-pointer"
                  ...
                  <Link href={`/product/${item.productId}`} className="relative w-full aspect-[4/5] overflow-hidden bg-brand-light/50 border-b border-brand/5 block">
                    ...
                    />
                    {item.totalStock === 0 && (
                      <div className="absolute bottom-0 left-0 w-full bg-[#faf8f0] py-2.5 z-20 border-t border-brand/5 text-center">
                  ...
                  <div className="pt-3 pb-3 px-3 flex flex-col flex-1">
                    ...
                    <p className="text-[10px] md:text-xs text-[#333333] font-bold uppercase tracking-widest mb-1.5">
                      {item.category}
                    </p>
                    ...
                    {item.totalStock === 0 ? (
                      <Link
                        ...
                        className="w-full mt-auto flex items-center justify-center space-x-2 font-bold py-3.5 rounded-2xl transition-all text-xs tracking-wider uppercase shadow-md bg-[#333333] text-white hover:bg-[#32451B] border border-transparent hover:border-brand-accent/20 cursor-pointer"
                      >
              ...
              <div className="p-6">
                ...
                {loadingDetails ? (
                  <div className="py-12 flex flex-col items-center justify-center">
                    <Loader2 className="w-8 h-8 animate-spin text-[#333333] mb-3" />
                  ...
                  productDetails && (
                    <div className="space-y-6">
                      ...
                      {availableColors.length > 0 && (
                        <div>
                          <span className="block text-xs font-black uppercase tracking-widest text-[#1B3022]/50 mb-3">
                            Select Color: <span className="text-[#333333] font-black">{selectedColor}</span>
                          </span>
                            ...
                            {availableColors.map((color) => (
                              <button
                                ...
                                className={`relative w-8 h-8 rounded-full border p-0.5 transition-all ${
                                  selectedColor === color
                                    ? "border-[#333333] scale-110 shadow-sm"
                      ...
                      {!isSingleSize && availableSizes.length > 0 && (
                        <div>
                          <span className="block text-xs font-black uppercase tracking-widest text-[#1B3022]/50 mb-3">
                            Select Size: <span className="text-[#333333] font-black">{selectedSize || "None"}</span>
                          </span>
                                ...
                                <button
                                  ...
                                  className={`w-12 h-12 rounded-full border flex items-center justify-center font-bold text-xs transition-all cursor-pointer ${
                                    isOutOfStock
                                      ...
                                      : selectedSize === size
                                      ? "bg-[#333333] text-white border-transparent scale-105 shadow-sm"
                                      : "border-[#1B3022]/10 bg-white text-[#1B3022] hover:border-[#333333]/50 hover:bg-[#333333]/5"
                                  }`}
                                >
                                  {size}
                                </button>
                      ...
                      <button
                        ...
                        className={`w-full py-4 rounded-2xl font-bold text-sm tracking-wider uppercase transition-all shadow-lg mt-8 flex items-center justify-center space-x-2 ${
                          !selectedColor || !selectedSize
                            ...
                            : "bg-[#333333] text-white hover:bg-[#32451B] active:scale-[0.98] cursor-pointer"
                        }`}
                      >
                        <Check size={16} />
                        <span>Done</span>
